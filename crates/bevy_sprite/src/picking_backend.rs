...
fn sprite_picking(
    ...
    let mut sorted_sprites: Vec<_> = sprite_query
        .iter()
        .filter_map(|(entity, sprite, transform, anchor, pickable, layers, vis)| {
            if !transform.affine().is_nan() && vis.get() {
                Some((entity, sprite, transform, anchor, layers, pickable))
            } else {
                None
            }
        })
    ...
    for (pointer, location) in pointers.iter().filter_map(|(pointer, pointer_location)| {
        ...
        let picks: Vec<(Entity, HitData)> = sorted_sprites
            ...
            .copied()
            .filter_map(
                |(entity, sprite, sprite_transform, anchor, sprite_layers, pickable)| {
                    if blocked {
                        return None;
                    }

                    if !camera_layers.intersects(sprite_layers.unwrap_or_default()) {
                        return None;
                    }
